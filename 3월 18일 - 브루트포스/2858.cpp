...
pair<int, int> length(int r, int b) {
    int area = r + b; // r과 b를 합쳐서 총 area 수이다. w*l이 area여야한다.
    for (int i = area; i > 0; i--) { //i = l(높이) 의 값
        if (area % i != 0) { //w 가 정수가 아니라면 넘어감
            continue; // area가 l로 나누어 떨어지지 않으면 w가 될 수 없기 때문에 pass 
        }
        int w = area / i; // w 구하기
        if (r == ((i + w) * 2 - 4)) { // 테두리 수를 구해서 빨간 타일 개수와 맞는지 확인
            return make_pair(i, w); //맞다면 정확한 l, w페어이므로 리턴
        }
    }
}
...
int main() {
    ...
    //연산
    pair<int, int> result = length(r, b); // 차례대로 방의 크기 L,W를 리턴하는 함수 length(r,b)를 호출한다.
